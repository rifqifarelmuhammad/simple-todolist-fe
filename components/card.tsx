        ...
        <div className="w-full transform transition duration-500 hover:scale-105" id="task-${data.pk}">
            <div className={"rounded-3xl p-4 " + color}>
                ...
                <div className="flex justify-center mt-6">
                    <button className='cursor-pointer bg-red-600 rounded-lg w-fit px-2' onClick={() => handleUpdate(id)}>Update</button>
                    <button className='ml-3 bg-indigo-300 rounded-lg w-fit px-2' onClick={() => handleDelete(id)}>Delete</button>
                </div>
            </div>
        </div>
    )
